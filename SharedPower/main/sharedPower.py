...
class CheckTools:
    ...
    def claim_insurance(self):
        print("\nYou will get according to the condition of the tools")
        choice=input("\nPress Enter or any key to redirect back to Home Page!! ")
        if choice=="":
            self.insurance_home()
        else:
...
class SearchTools(CheckTools):
    def search(self):
        conn = sqlite3.connect('project.db')
        c = conn.cursor()
        self.name_of_tool = input("Enter ToolName: ")
        #self.tool_type = input("Enter Type: ")
        c.execute('SELECT * FROM ToolsInfo WHERE NameOfTool = ?',(self.name_of_tool,))
        conn.commit()
        data = c.fetchall()
        if data:
            for elem in data:
...
class Registration(Login):
    def create_table2(self):
        c.execute('CREATE TABLE IF NOT EXISTS UserInfo(Firstname NOT NULL,Lastname NOT NULL, Email UNIQUE NOT NULL, Username UNIQUE NOT NULL, Password NOT NULL, Date NOT NULL, Time NOT NULL)')
    ...
    def user_info(self):
        ...
        if len(self.username)<=5 and len(self.password)<=5:
            ...
            choice = input('\nPress Enter to Retry!! or Press any key to go back!!')
            if choice=='':
                self.user_info()
            else:
                self.display_text()
        else:
            c.execute('SELECT * FROM UserInfo WHERE Username=? OR Email=?',(self.username,self.email))
            conn.commit()
            if c.fetchall():
                ...
                c.execute('INSERT INTO UserInfo (Firstname, Lastname, Email, Username, Password, Date, Time) VALUES(?,?,?,?,?,?,?)',
                      (self.firstname, self.lastname, self.email, self.username, self.password, self.current_date, self.current_time))
                conn.commit()
